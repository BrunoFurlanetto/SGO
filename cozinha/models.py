...
class HorarioRefeicoes(models.Model):
    ...
    def __str__(self):
        return f'Horario padrão do {self.refeicao}'

    @classmethod
    def refeicoes_padrao(cls):
        """
        Retorna a lista de opções de refeições padrão.
        """
        return dict(cls.refeicoes)

    @classmethod
    def horarios(cls):
        horas_bd = cls.objects.all()
        horario_refeicoes = {}
...
class RegistroVisualizacoes(models.Model):
    usuario = models.ForeignKey(User, on_delete=models.CASCADE)
    data_refeicoes = models.DateField()
    data_hora_visualizacao = models.DateTimeField(auto_now_add=True, editable=False)
